...
<?php 
    ...
    while ($i <= 100) {
        if ($i % 3 == 0) {
            echo "$i - это число делятся на 3 без остатка </br>";
            $i++;
        } else {
            $i++;
        }
...
<?php
    $region = [   
        ...
        "Московска область" => ["Москва", "Зеленоград", "Клин"]
    ];
...
<!-- *Объединить две ранее написанные функции в одну, которая получает строку на русском языке, производит транслитерацию и замену пробелов на подчеркивания (аналогичная задача
...
<?php
    function translitText($text) {
        ...
        $newtext = strtolower(strtr($text, $trans));
        $newtext = trim(preg_replace("/\s+/", "_", $newtext),"_");
        return $newtext;
    };

    echo translitText(" Привет О дивный 1234 Новый мир ");
?>
<hr>
